...
standard_batches = batch_gid_map("data/batches")
kdd_batches = batch_gid_map("data/kdd_batches")

missing_batches, missing_kdd_batches, missing_gids = set(), set(), []
for p in analysis:
    gid = str(p["gid"])
    if complete(rich.get(gid), PAPER_FIELDS):
        continue
    missing_gids.append(gid)
    if gid in kdd_batches:
        missing_kdd_batches.add(kdd_batches[gid])
    else:
        missing_batches.add(standard_batches.get(gid, f"gid:{gid}"))
...
print("kdd_workflow batches: ", " ".join(sorted(missing_kdd_batches)) or "none")
