export const meta = {
  name: 'dm-deep-concepts',
  description: 'Deep first-principles conceptual essays for recurring math ideas and paper families (Haiku, per concept)',
...
// Resume controls. Leave ONLY empty to run all concepts; otherwise list keys like ['rank','probability'].
const ONLY = []
const WAVE_SIZE = 4
...
The six parts:
  "idea"    = THE IDEA, FROM SCRATCH: the real-world need that forces this idea into existence, and
              what the idea actually IS once you strip away all notation. 8-12 sentences.
  "why"     = WHY IT WORKS: the underlying reason/guarantee that makes it valid — not a slogan, the
              actual mechanism of why it works and where it breaks. 7-10 sentences.
  "math"    = THE MATHEMATICAL PRINCIPLE: the important mathematical object or move, explained without
              prior math knowledge. Say what is being measured, compared, optimized, ranked, sampled,
              matched, constrained, compressed, or protected, and why that matters. 8-12 sentences.
...
log(`DM concept essays: ${results.filter(Boolean).length}/${jobs.length} returned`)
